...
try {
  const result = await runFeatures(
    toFileUrl(Deno.cwd()),
    parseFeatures(Deno.args, builtInFeatureRegistry),
  );
  console.log(result);
} catch (error) {
