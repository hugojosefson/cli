...
test("task features use exact commands and canonical aggregates", () => {
...
test("every task subset has canonical check dependencies", () => {
...
test("real runner composes and removes leaf tasks from one snapshot", async () => {
  ...
  try {
    ...
    config = JSON.parse(await readTextFile(new URL("deno.jsonc", root)));
    ...
    assertEquals(
      (await run(root, [])).split("\n").slice(2).filter((line) =>
        /^\S/.test(line)
      ).map((line) => line.trim().split(/ +/).slice(0, 2).join(": ")).join(
        "\n",
      ),
